# Self-Learning Activity – Agile, DevOps & Requirement Engineering

This repository contains my Self-Learning Activity on Agile, DevOps, JIRA, Asana, and Requirement Engineering.

## Case Study

A **College Canteen Management System** is used as a practical context throughout the activity to understand how the concepts can be applied in a real software project.

## Topics Covered

1. Comparison of Agile vs DevOps
2. Case Study: JIRA vs Asana in Real Projects
3. Writing Effective User Stories and Acceptance Criteria
4. Advanced Requirement Elicitation Techniques
5. Requirement Traceability Matrix (RTM)
6. Tools for Requirement Management


## References

- AWS — What is DevOps?
- Atlassian — JIRA Software
- Asana — Project Management
- IBM — IBM Engineering Requirements Management DOORS
- Agile Manifesto
